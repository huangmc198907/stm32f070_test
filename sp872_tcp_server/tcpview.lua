...
function ServerClientView:__init(socket)
    ...
    self.sendBtn10k = QPushButton("Send 10k")
    ...
    self.iot = IotView(self)

    subWindow = mdiArea:addSubWindow( self.iot )
    subWindow:show()
    ...
    self.layout = QVBoxLayout{
            ...
            QVBoxLayout{
                ...
                self.recvEdit,
                QHBoxLayout{ QLabel("Send buf:"), self.sendBtn, self.sendBtn10k, QLabel(""), strech = "0,0,0,1" },
                self.sendEdit,
            },
            strech = "0,1",
        }
    ...
    self.sendBtn10k.clicked = function()
        self.socket:write(data10K)
    end
    ...
    self.socket.readyRead = function()
        ...
        local data = self.socket:readAll()
        self.recvEdit:append(data)
        self.recvEdit:scrollToEnd()
        if self.readyRead then
            self.readyRead(data)
        end
    end
...
function TcpClientView:__init()
    ...
    self.chkDontRead = QCheckBox("Don't Read")
    self.recvEdit = QHexEdit{
        overwriteMode = false,
        readOnly = true,
    }
    ...
    self.layout = QVBoxLayout{
            self.settings,
            QVBoxLayout{
                QHBoxLayout{ QLabel("Recv buf:"), self.clearBtn, self.chkDontRead, QLabel(""), strech = "0,0,0,1" },
                self.recvEdit,
    ...
    self.tcpClient.readyRead = function()
        if self.chkDontRead.checked then
