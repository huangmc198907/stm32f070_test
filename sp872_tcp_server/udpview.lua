...
function UdpView:__init()
    ...
    self.send_data = function(data)
        local addr = QHostAddress(self.sendIP.text)
        local port = tonumber(self.sendPort.text)
        self.udp:writeDatagram(data,addr,port)
    end
    ...
    self.udp.readyRead = function()
        while self.udp.hasPendingDatagrams do
            local size = self.udp.pendingDatagramSize
            local data,fromAddr,fromPort = self.udp:readDatagram(size)
