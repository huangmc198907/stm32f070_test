...
-- split the string with specified separator, in separator is nil, set it to %s (lua defined separators)
function split(str, sep)
    local r = {}
    sep = sep or "%s"
    string.gsub(str, "([^"..sep.."]+)", function(t)
        r[#r+1] = t
    end)
    return r
end

function append(tb, data)
    tb = tb or {}
    tb[#tb+1] = data
    return tb
end
...
function _G.toCodecString(data, codec)
    if not codec then
        return QUtil.toString(data)
    end
    local c = QTextCodec.codecForName(codec)
    if not c then
        return QUtil.toString(data)
    end
